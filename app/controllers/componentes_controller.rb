class ComponentesController < ApplicationController
  ...
  def index
    @componentes = Componente.order(:id)
  end
...
# Inactivar atributos
  def inactivar_componente
    ...
    respond_to do |format|
      if @componente.save
        format.html { redirect_to componentes_path, notice: "Componente Inactivado" }
        format.json { render :show, status: :created, location: @componente }
      else
        format.html { redirect_to componentes_path, alert: "Ocurrio un error al inactivar Componente, Verifique!!..." }
        format.json { render json: @componente.errors, status: :unprocessable_entity }
      end
    end
  end
  ...
  def activar_componente
    ...
    respond_to do |format|
      if @componente.save
        format.html { redirect_to componentes_path, notice: "Componente Activado" }
        format.json { render :show, status: :created, location: @componente }
      else
        format.html { redirect_to componentes_path, alert: "Ocurrio un error al activar Componente, Verifique!!..." }
        format.json { render json: @componente.errors, status: :unprocessable_entity }
      end
    end
  end
